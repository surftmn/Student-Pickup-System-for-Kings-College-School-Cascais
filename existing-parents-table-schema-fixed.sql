-- Kings College School - Schema for Existing Parents Table with Student IDs (FIXED VERSION)
-- This works with your existing parents table structure and adds login functionality
-- WITHOUT complex RLS policies that cause auth.users errors
...
-- Step 4: Create function for parent login authentication (PUBLIC ACCESS)
...
RETURNS TABLE (
    success BOOLEAN,
    parent_id BIGINT,  -- Changed to BIGINT
    parent_name TEXT,  -- Changed to TEXT to match your table
    student_ids INTEGER[],
    message TEXT
) AS $$
DECLARE
...
BEGIN
    ...
    ELSE
        RETURN QUERY SELECT FALSE, NULL::BIGINT, NULL::TEXT, NULL::INTEGER[], 'Invalid email or password'::TEXT;
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Step 5: Create function to get parent's children details (PUBLIC ACCESS)
...
-- Step 6: Create function to find student ID by name
...
-- Step 7: Create function to process CSV import and update parents
...
RETURNS TABLE (
    success BOOLEAN,
    parent_id BIGINT,  -- Changed to BIGINT to match your table
...
DECLARE
    parent_record_id BIGINT;  -- Changed to BIGINT
    child_name TEXT;
...
BEGIN
    ...
    -- Insert or update parent record (matching your exact table structure)
    INSERT INTO parents (
        name, 
        email, 
        password, 
...
EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT FALSE, NULL::BIGINT, NULL::INTEGER[], NULL::TEXT[], NULL::VARCHAR, SQLERRM::TEXT;
END;
$$ LANGUAGE plpgsql;

-- Step 8: Create helper view for parent management
...
-- Step 9: Insert sample data based on your CSV
...
-- Step 10: Grant necessary permissions (SIMPLIFIED - NO RLS FOR NOW)
GRANT USAGE ON SCHEMA public TO anon, authenticated;
GRANT ALL ON parents TO anon, authenticated;
GRANT ALL ON students TO anon, authenticated;
GRANT SELECT ON parent_management_view TO anon, authenticated;
...
-- Step 12: Success message and testing queries
DO $$ 
BEGIN
    RAISE NOTICE '✅ Parent login system setup complete with existing parents table!';
    RAISE NOTICE '';
    RAISE NOTICE '🚨 SECURITY NOTE: RLS policies disabled for now - add them later when auth is configured';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '4. Process new parent: SELECT * FROM process_parent_csv_import(''Test Parent'', ''test@email.com'', ARRAY[''Test Child'']);';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '';
    RAISE NOTICE '🔧 Next: Use parent-csv-uploader.html to import your real data!';
END $$;
