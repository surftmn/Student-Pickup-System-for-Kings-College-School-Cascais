// School Pickup System with Supabase Integration

class SchoolPickupSystem {
    constructor() {
        this.students = [];
        this.queue = [];
        this.isOnline = false;
    ...
    async initializeDatabase() {
        this.showToast('Connecting to database...', 'info');
        ...
        try {
            ...
            if (connectionTest.needsSetup) {
                this.showToast('Database setup required - check console for instructions', 'warning');
            ...
            this.isOnline = true;
            this.showToast('Connected! Loading data...', 'success');
            ...
            this.showToast('Real-time sync enabled!', 'success');
            
        } catch (error) {
            console.error('Database initialization failed:', error);
            this.isOnline = false;
            this.showToast('Offline mode - using local storage', 'warning');
    ...
    async loadAllData() {
        try {
            ...
            if (studentsResult.success) {
                this.students = studentsResult.data.map(student => ({
                    id: student.id,
                    name: student.name,
                    grade: student.grade,
            ...
            // Load pickup queue
            const queueResult = await this.db.getPickupQueue();
            if (queueResult.success) {
                this.queue = queueResult.data.map(item => ({
                    id: item.id,
            ...
            // If no students exist, offer to load demo data
            if (this.students.length === 0) {
                this.showDemoDataPrompt();
            }
    ...
    setupRealTimeSubscriptions() {
        ...
        this.db.subscribeToStudents((payload) => {
            console.log('Students updated:', payload);
            this.handleStudentUpdate(payload);
        });

        // Subscribe to pickup queue changes
        this.db.subscribeToPickupQueue((payload) => {
            console.log('Pickup queue updated:', payload);
            this.handleQueueUpdate(payload);
        });
    }

    async handleStudentUpdate(payload) {
        // Reload students data and re-render
        const studentsResult = await this.db.getStudents();
        if (studentsResult.success) {
            this.students = studentsResult.data.map(student => ({
                id: student.id,
    ...
    async handleQueueUpdate(payload) {
        // Reload queue data and re-render
        const queueResult = await this.db.getPickupQueue();
        if (queueResult.success) {
            this.queue = queueResult.data.map(item => ({
                id: item.id,
    ...
    showDemoDataPrompt() {
        const shouldLoad = confirm('No students found. Would you like to load demo data to get started?');
        if (shouldLoad) {
            this.loadDemoData();
        }
    }
    ...
    async loadDemoData() {
        if (this.isOnline) {
            // Use Supabase to seed demo data
            const result = await this.db.seedDemoData();
            if (result.success) {
    ...
    async handleParentCheckin() {
        ...
        if (this.isOnline) {
            // Add to Supabase
            const result = await this.db.addToPickupQueue(queueEntry);
            if (result.success) {
    ...
    // Complete a pickup
    async completePickup(queueId) {
        if (this.isOnline) {
            const result = await this.db.updatePickupStatus(queueId, true);
            if (result.success) {
    ...
    async removeFromQueue(queueId) {
        if (this.isOnline) {
            const result = await this.db.removeFromPickupQueue(queueId);
            if (result.success) {
                const queueItem = this.queue.find(item => item.id === queueId);
                if (queueItem) {
                    this.showToast(`Removed ${queueItem.parentName} from queue`, 'warning');
                }
            } else {
    ...
    async addStudent() {
        ...
        if (this.isOnline) {
            const result = await this.db.addStudent(newStudent);
            if (result.success) {
                this.hideAddStudentModal();
    ...
    async deleteStudent(studentId) {
        if (confirm('Are you sure you want to delete this student?')) {
            if (this.isOnline) {
                ...
                if (result.success) {
                    const deleted = this.students.find(s => s.id === studentId);
                    if (deleted) {
    ...
    async clearQueue() {
        if (confirm('Are you sure you want to clear the entire pickup queue?')) {
            if (this.isOnline) {
                const result = await this.db.clearPickupQueue();
                if (result.success) {
    ...
    async exportData() {
        if (this.isOnline) {
            const result = await this.db.exportAllData();
            if (result.success) {
